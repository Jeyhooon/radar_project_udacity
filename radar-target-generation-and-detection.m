...
B = c/(2*range_res);
tau = 2 * max_range / c;  % round trip time
Tchirp = 5.5 * tau;
slope = B / Tchirp;
...
%The fftshift function is used to shift the zero-frequency component to 
% the center of the spectrum. This is often done for visualisation purposes
sig_fft2 = fftshift (sig_fft2); 
RDM = abs(sig_fft2);
RDM = 10*log10(RDM);
%result is then converted to logarithmic scale (in decibels), which can help to 
% visualize the result because the power values can vary over several orders of magnitude.
...
% set those values to 0. 
 % This is already done in the for-loop above; 
 % by first initializing the CFAR with zeros and then adding values to it 
